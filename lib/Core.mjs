...
export default class Core {
  ...
  static create(data = {}, caller = undefined) {
    ...
    if(caller)
      Log.info({ message: `${caller.name} creates ${instance.name} instance`, source: this.name });
  ...
  static resolvePath(data) {
    return data.startsWith('#/') ? path.resolve(this.path, '..', data.substr(2)) : path.resolve(this.path, data);
  }
}
