...
export default class Core {
  ...
  constructor(data = {}) {
    ...
    this.event = new Namespace.FrameworkEvent(this);
    this.#name = this.__proto__.constructor.name;
    Core.load(data)
      .then((data) => {
        this.data = Object.assign({}, data);
      }).catch((error) => {
        new Namespace.FrameworkError(error);
      }).finally(() => {
  ...
  static apply(data) {
    return new Promise((resolve, reject) => {
      this.load(data).then((data) => {
        resolve(Object.assign(this.default, data));
      }).catch((error) => {
        reject(error);
      });
    });
