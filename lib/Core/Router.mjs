...
export default class Router extends Namespace.Core {
  ...
  parse(request) {
    return new Promise((resolve, reject) => {
      Namespace.Log.info({ message: `Router parses request`, source: this.name, payload: `${request.method} ${request.path}` });
      ...
      if(foundRoute && parsedData.method != 'skip') {
        ...
        Namespace.Log.info({ message: `Router returns results`, source: this.name, payload: data });
        resolve(Namespace.JResponse.success({ data: data }));
      } else {
        reject(Namespace.JResponse.respond('notAllowed', { message: 'Router does not recognize this route' }));
      }
    });
  }
}
