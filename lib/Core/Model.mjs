...
export default class Model extends Namespace.Core {
  ...
  method = {
    ...
    get: function() {
      return new Promise((resolve, reject) => {
        ...
        this.set('driver', this.controller.source.driver, Namespace.Model.default.driver);
        // Invoke the driver
        Namespace.Driver.create(this.prepareData(), this)
          .then((driver) => {
            // Copy source object from controller
            driver.source = this.controller.source;
            // Invoke driver method
            return driver.invokeMethod();
          }).then((response) => {
            Namespace.Log.info({ message: `Model returns useful data`, source: this.name, payload: response });
            // Resolve response
            resolve(response);
          }).catch((error) => {
            Namespace.Log.info({ message: `Model returns useless data`, source: this.name, payload: error });
            // Reject with error
            reject(error);
          });
      });
  ...
  prepareData() {
    ...
    data['page'] = Namespace.Model.parseInt(this.get('page')) || Model.default.page;
    data['pageSize'] = Namespace.Model.parseInt(this.get('pageSize')) || Model.default.pageSize;
    data['show'] = Namespace.Model.parseArray(this.get('show')) || Model.default.show;
    data['hide'] = Namespace.Model.parseArray(this.get('hide')) || Model.default.hide;
    data['sort'] = Namespace.Model.parseArray(this.get('sort')) || Model.default.sort;
