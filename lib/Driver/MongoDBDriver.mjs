...
const method = {
  ...
  get: function() {
    return new Promise((resolve, reject) => {
      const client = new MongoClient(this.source.uri);
      client.connect().then(() => {
        let collection = client.db(this.source.database).collection(this.source.collection);
        let result;
        if(this.get('id')) {
...
//const uri = 'mongodb+srv://public:PapiaKuMi@cluster0.chacl.mongodb.net/cubo-cms?w=majority';
//const dbName = 'cubo-cms';
