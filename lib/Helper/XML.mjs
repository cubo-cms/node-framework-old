...
export default class XML extends Namespace.Core {
  ...
  /** @static @function isXML(data)
    ...
    **/
  static isXML(data) {
    return XMLParser.validate(data) === true;
  }
        ...
        if(this.isURL(data)) {
          ...
          scheme.get(data, (response) => {
            ...
            response.on('data', (chunk) => {
              ...
              if(this.isXML(payload)) {
                parsedData = XMLParser.parse(payload, { ignoreAttributes:false, attributeNamePrefix: '' });
              } else {
          ...
          fs.readFile(filePath, 'utf8', (error, data) => {
            if(error) {
              ...
              let parsedData;
              if(this.isXML(data)) {
                parsedData = XMLParser.parse(data, { ignoreAttributes:false, attributeNamePrefix: '' });
              } else {
                throw new Namespace.FrameworkError('No JSON or XML');
              }
              resolve(parsedData);
